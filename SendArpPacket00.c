...
int main() {
	...
	ether_hd->ether_shost[0] = 0xaa;
	ether_hd->ether_shost[1] = 0xaa;
	ether_hd->ether_shost[2] = 0xaa;
	ether_hd->ether_shost[3] = 0xaa;
	ether_hd->ether_shost[4] = 0xaa;
	ether_hd->ether_shost[5] = 0xaa;
	...
	sender_mac->ether_addr_octet[0] = 0xaa;
	sender_mac->ether_addr_octet[1] = 0xaa;
	sender_mac->ether_addr_octet[2] = 0xaa;
	sender_mac->ether_addr_octet[3] = 0xaa;
	sender_mac->ether_addr_octet[4] = 0xaa;
	sender_mac->ether_addr_octet[5] = 0xaa;

	sender_addr = inet_addr("192.168.0.10");

	for (i = 0; i < 6; i++) {
		target_mac->ether_addr_octet[i] = 0x00;
	}

	target_addr = inet_addr("192.168.0.20");

	memcpy(ArpPacket, ether_hd, sizeof(struct libnet_ethernet_hdr));
	memcpy(ArpPacket + 14, arp_hd, sizeof(struct libnet_arp_hdr));
	memcpy(ArpPacket + 14 + 28, sender_mac, sizeof(struct libnet_ether_addr));
	memcpy(ArpPacket + 14 + 28 + 6, sender_addr, sizeof(struct in_addr));
	memcpy(ArpPacket + 14 + 28 + 6 + 4, target_mac, sizeof(struct libnet_ether_addr));
	memcpy(ArpPacket + 14 + 28 + 6 + 4 + 6, target_addr, sizeof(struct in_addr));
